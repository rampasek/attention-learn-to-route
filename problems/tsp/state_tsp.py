...
class StateEdgeTSP(NamedTuple):
    ...
    def get_final_cost(self):
        assert False, "Unused, instead implementation in a problem class, e.g. TSPEdge, of get_costs is used."
        assert self.all_finished()
        return self.lengths + (self.loc[self.ids, self.first_a, :] - self.loc[self.ids, self.prev_a, :]).norm(p=2, dim=-1)

    def update(self, edge_start, edge_end):

        # Update the state
        start_node = edge_start[:, None]
        end_node = edge_end[:, None]

        # Add the length
        start_coord = self.loc[self.ids, start_node]
        end_coord = self.loc[self.ids, end_node]
        lengths = self.lengths + (end_coord - start_coord).norm(p=2, dim=-1)  # (batch_dim, 1)

        # Update should only be called with just 1 parallel step, in which case we can check this way if we should update
        first_a = start_node if self.i.item() == 0 else self.first_a

        visited_ = self.visited
        if self.visited_.dtype == torch.uint8:
            # Add one dimension since we write a single value
            visited_ = visited_.scatter(-1, start_node[:, :, None], 1)
            # visited_ = visited_.scatter(-1, end_node[:, :, None], 1)
        else:
            visited_ = mask_long_scatter(visited_, start_node)
            # visited_ = mask_long_scatter(visited_, end_node)

        return self._replace(first_a=first_a, prev_a=end_node, visited_=visited_,
